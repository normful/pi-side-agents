...
const DEFAULT_WAIT_STATES: AgentStatus[] = [
	"waiting_user",
	"failed",
	"crashed",
];
...
function resolveBacklogPathForRecord(
	stateRoot: string,
	record: AgentRecord,
): string {
...
function normalizeWaitStates(input?: string[]): {
	values: AgentStatus[];
	error?: string;
} {
...
async function prepareFreshRuntimeDir(
	stateRoot: string,
	agentId: string,
...
async function updateWorktreeLock(
	worktreePath: string,
...
async function cleanupWorktreeLockBestEffort(
	worktreePath?: string,
): Promise<void> {
...
async function scanOrphanWorktreeLocks(
	repoRoot: string,
	registry: RegistryFile,
...
async function reclaimOrphanWorktreeLocks(
...
async function syncParallelAgentPiFiles(
...
async function buildKickoffPrompt(
	ctx: ExtensionContext,
	task: string,
	includeSummary: boolean,
...
function splitModelPatternAndThinking(raw: string): {
	pattern: string;
	thinking?: string;
} {
...
async function agentCheckPayload(
	stateRoot: string,
	agentId: string,
...
async function setChildRuntimeStatus(
	ctx: ExtensionContext,
	nextStatus: AgentStatus,
): Promise<void> {
...
async function ensureChildSessionLinked(
	pi: ExtensionAPI,
	ctx: ExtensionContext,
): Promise<void> {
	const agentId = process.env[ENV_AGENT_ID];
...
function emitKickoffPromptMessage(
	pi: ExtensionAPI,
